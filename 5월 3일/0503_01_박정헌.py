...
'''
1. 인덱싱(indexing) : 순차적인 자료구조에 인덱스(첨자) 값을 가지고 접근할 수 있는 기능.
    [i] => i번째 값.
'''
surName = name[0]   # name 문자열에서 첫번째 문자를 변수에 저장.
...
'''
2. 슬라이싱(slicing) : 시퀸스 자료형에서 일부를 잘라내어 동일한 자료형으로 반환하는 기능.
[start : stop : step] => start에서 시작해서 step 이전까지 step 간격으로 추출.
'''
...
# city(리스트) + num1(튜플)
#result = city + num1    # 오류 발생. (이유 : 서로 다른 자료형은 합칠 수 없다.)
...
# in 연산자는 for 반복문에 효율적으로 사용할 수 있다.
